 ...
 *
 * 对源代码修改
 * 此类引用了一些限定只能在org.apache.spark.mllib包内方法和变量，最好不要改动包路径
 *
 */
...
class JGMM  (
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    JGMM.SaveLoadV1_0.save(sc, path, weights, gaussians)
  }
...
//      case (weight, dist) => MLUtils.EPSILON + weight * math.exp(dist.logpdf(pt) / 10)
...
object JGMM extends Loader[JGMM] {
  ...
  private object SaveLoadV1_0 {
    ...
    def load(sc: SparkContext, path: String): JGMM = {
      ...
      new JGMM(weights.toArray, gaussians.toArray)
    }
  }


  override def load(sc: SparkContext, path: String): JGMM = {
