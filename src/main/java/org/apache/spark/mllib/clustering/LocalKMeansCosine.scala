...
/**
  ...
  */
private[mllib] object LocalKMeansCosine extends Logging {
  ...
  def kMeansPlusPlus(
    ...
    for (i <- 1 until k) {
      ...
      val sum = points.view.zip(weights).map { case (p, w) =>
        w * KMeansCosine.pointCost(curCenters, p)
      }.sum
      val r = rand.nextDouble() * sum
      var cumulativeScore = 0.0
      var j = 0
      while (j < points.length && cumulativeScore < r) {
        cumulativeScore += weights(j) * KMeansCosine.pointCost(curCenters, points(j))
        j += 1
      }
      if (j == 0) {
    ...
    while (moved && iteration < maxIterations) {
      ...
      while (i < points.length) {
        val p = points(i)
        val index = KMeansCosine.findClosest(centers, p)._1
