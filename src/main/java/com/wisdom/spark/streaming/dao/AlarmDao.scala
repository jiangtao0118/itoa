...
//告警阈值配置信息保存
  ...
  //更新指标规则配置关系表
  def updateRNcoCrel(conn: Connection): Unit ={
    var pst1: PreparedStatement = null
    var pst2: PreparedStatement = null
    val batchNums = props.getProperty("mysql.sql.batch").toInt
    val sql1 ="delete from t_alarm_rules_conf_rel where sysName in ('reporter_status') "
    val sql2 = "INSERT INTO t_alarm_rules_conf_rel ( ruleId,confId,sysName) select ?,confId,sysName from t_alarm_configuration where sysName in ('reporter_status') "
    try {
      pst1 = conn.prepareStatement(sql1)
      pst2 = conn.prepareStatement(sql2)
      pst1.executeUpdate()
  ...
  //更新指标规则配置关系表
  def updateRItmCrel(conn: Connection): Unit ={
    var pst1: PreparedStatement = null
    var pst2: PreparedStatement = null
    val batchNums = props.getProperty("mysql.sql.batch").toInt
    val sql1 ="delete from t_alarm_rules_conf_rel where sysName in ('System','Application_Server','Unix_Memory') "
    val sql2 = "INSERT INTO t_alarm_rules_conf_rel ( ruleId,confId,sysName ) select ?,confId,sysName from t_alarm_configuration where sysName in ('System','Application_Server','Unix_Memory') "
    try {
      pst1 = conn.prepareStatement(sql1)
      pst2 = conn.prepareStatement(sql2)
      pst1.executeUpdate()
  ...
  //更新指标规则配置关系表
  def updateROmpCrel(conn: Connection): Unit ={
    var pst1: PreparedStatement = null
    var pst2: PreparedStatement = null
    val batchNums = props.getProperty("mysql.sql.batch").toInt
    val sql1 ="delete from  t_alarm_rules_conf_rel where sysName in ('OPM_DB') "
    val sql2 = "INSERT INTO t_alarm_rules_conf_rel ( ruleId,confId,sysName ) select ?,confId,sysName from t_alarm_configuration where sysName in ('OPM_DB')"
    try {
      pst1 = conn.prepareStatement(sql1)
      pst2 = conn.prepareStatement(sql2)
      pst1.executeUpdate()
  ...
  def findLatelyPredResult(res: PredResult, conn: Connection): List[String] = {
    val sql = "SELECT t1.currentActualValue FROM t_pred_result AS t1 WHERE t1.hostName = ? AND t1.predIndexName = ? AND t1.predPeriod = ? AND t1.currentDataTime <= ? AND t1.currentDataTime >= ?- 3600 ORDER BY t1.predId DESC LIMIT 0, 5"
