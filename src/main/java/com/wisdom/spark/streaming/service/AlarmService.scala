...
//  }
  ...
  def ruleFunc9(res: PredResult,conn:Connection): AlarmResult = {
    val lv = alarmDao.findLatelyPredResult(res, conn)
    val currentValue = res.getCurrentActualValue.toDouble
    try {
      if (lv != null && lv.size>=5) {
        val m0=currentValue - lv(0).toDouble
        ...
        //val m04=lv(0).toDouble - lv(4).toDouble
        val m3 =lv(3).toDouble
        if (m0-m01 > 0 && m01-m12 >0 && m12-m23 >0 && currentValue > 4*m3) {
