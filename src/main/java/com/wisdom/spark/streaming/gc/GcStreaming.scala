...
//    正常数据所占比例
    ...
    for (hostName <- host_names) {
      try {
        ...
        val host_gmm = GaussianMixtureModel.load(sc, ml_path.concat(hostName.toString + "/gmm"))
        //通过读文件，获取用于模型训练的列索引。返回数组
        val corr_index = sc.textFile(ml_path.concat(hostName.toString + "/mid/corr.csv")).map(_.toInt).collect()
        //计算异常高斯的索引
        val abnormalIdx = getMinorityGaussianIndexs(host_gmm, normalPercent.toDouble)
        // Map( 主机->(gmm,异常高斯的索引,降维后的列索引) )
        model_maps += (hostName.toString ->(host_gmm, abnormalIdx, corr_index))
      } catch {
